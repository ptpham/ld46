  ...
  renderModal() {
    ...
    return html`
      <modal-frame>
        ...
        connected blob's health drops below zero, it dies. When a blob gets
        an additional hex, it's health increases. A blob's health can not be
        greater than the number of hexes that compose it. Two blobs connected
        by an edge will take on the maximum health between them. Two adjacent
        hexes of the same color are connected if the edge between them is
        claimed by the same player or is unclaimed.  A hex is connected to an
        edge if the edge is one of the hexes edges.  Edges are connected if
        they share a corner.
