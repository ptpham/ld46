...
import { hexCenterXFunction, hexCenterYFunction } from "./hex-grid.js";
import { validCoords } from "./hex-grid.js";
...
export class HexaSlimeGame extends LitElement {
  ...
  getClickableEdgeIdsForCurrentPlayer() {
    ...
    return [...hexes.flatMap(x => hexGrid.hexAdjacentEdges(x)), ...edges.flatMap(x => hexGrid.halfedgesAdjacentEdges(x))].map(x => hexGrid.coordToId(x)).filter(id => {
