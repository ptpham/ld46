...
export class HexGrid extends LitElement {
  ...
  dispatchGenericMouseEvent(e, name) {
    ...
    if (tileType) {
      ...
      this.dispatchEvent(new CustomEvent(`tile-${name}`, {
        detail
      }));
    }
  }
  ...
  halfedgesAdjacentEdges(coord) {
    ...
    return [this.halfedgeNext(coord), this.halfedgePrev(mirror), this.halfedgePrev(coord), this.halfedgeNext(mirror)].map(x => this.canonicalHalfedge(x));
  }
